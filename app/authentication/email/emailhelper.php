...
function sendemailverify($username, $email, $token)
{
    ...
    $email_template = "
        ...
        <a href=  'http://localhost/sparshblogs/app/authentication/verifyemail.php?token=$token'>Verify Email</a>
        ";
...
function sendpasswordreset($email, $token)
{
    ...
    $email_template = "
            ...
            <a href= 'http://localhost/sparshblogs/app/authentication/changepass.php?token=$token&email=$email'>Reset Password</a>
            ";
    $mail->Body = $email_template;
    $mail->send();
}
